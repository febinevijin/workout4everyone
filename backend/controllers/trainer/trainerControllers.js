...
import fs from 'fs'
import Path from 'path';
import { uploadImageFile, videoUploader, uploadVideoFile } from '../../middleware/fileHandler.js';
...
// console.log(image);
const profileUpload = await cloudinary.v2.uploader.
upload(profileImage,{
   ...
   let trainerProof= uploadResponse.secure_url
    ...
    const trainer = await Trainer.create({
        name,email,password,trainerProfile,trainerProof
    })
...
export const postTrainerWorkoutVideo = async (req,res,next) =>{
...
try {
        ...
        const video = await uploadVideoFile(req.files.video,next)
        console.log(video,"----------");
